...
describe('E1-4: KnowledgeProcessor — visualResult in result', () => {
  it('KnowledgeProcessorResult has visualResult field (can be null)', async () => {
    ...
    const { KnowledgeIntelligenceDomain } = await import('../../../src/domains/KnowledgeIntelligenceDomain');
    const processor = new KnowledgeProcessor(new KnowledgeIntelligenceDomain(db), bus);
