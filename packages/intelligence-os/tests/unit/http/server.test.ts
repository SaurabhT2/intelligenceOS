...
import { createCognitionHttpServer } from '../../../src/api/http/server';
...
describe('createCognitionHttpServer', () => {
  ...
  function start(p: ReturnType<typeof makeMockProvider>): Promise<void> {
    provider = p;
    server = createCognitionHttpServer(provider, { apiKey: API_KEY, logger: { info: vi.fn(), warn: vi.fn(), error: vi.fn() } });
