...
function makeObservation(overrides: Partial<Observation> = {}): Observation {
  return {
    signalId:        'sig-001',
    userId:          'user-001',
    subject:         { subjectType: 'user', subjectId: 'user-001' },
    subjectType:     'user',
    workspaceId:     null,
    projectId:       null,
...
describe('HypothesisEngine', () => {
  it('creates a PROVISIONAL hypothesis when none exists', async () => {
    ...
    const engine = new HypothesisEngine(new UserIntelligenceDomain(db as unknown as import('@supabase/supabase-js').SupabaseClient));
...
describe('LearningValidator', () => {
  it('does not promote when below corroboration threshold', async () => {
    ...
    const db = createMockSupabase();
    const validator = new LearningValidator(new UserIntelligenceDomain(db as unknown as import('@supabase/supabase-js').SupabaseClient));
...
describe('FeedbackProcessor', () => {
  ...
  describe('processObservation() (ADR-003)', () => {
    it('runs a workspace observation through the full Learning Pipeline and reports a workspace subject', async () => {
      const { processor } = makeProcessor();

      const result = await processor.processObservation({
        workspaceId: 'ws-001',
        requestId: 'req-001',
        outputText: 'Generated artifact text.',
        score: 0.82,
      });

      expect(result.subject).toEqual({ subjectType: 'workspace', subjectId: 'ws-001' });
      expect(result.signalsProcessed).toBeGreaterThan(0);
      expect(result.observationsCreated).toBeGreaterThan(0);
      expect(result.errors).toEqual([]);
    });

    it('produces zero signals (and never throws) for a placeholder score of 0', async () => {
      const { processor } = makeProcessor();
    ...
    it('never throws even when a stage fails, and records the failure in result.errors', async () => {
      const db = createMockSupabase({
