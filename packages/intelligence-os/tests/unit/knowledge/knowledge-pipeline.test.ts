...
describe('KnowledgeProcessor', () => {
  ...
  function makeProcessor(returnedData?: Record<string, unknown>) {
    ...
    return { processor: new KnowledgeProcessor(chain as unknown as import('@supabase/supabase-js').SupabaseClient, bus), db: chain };
  }
  ...
  it('handles DB persistence failure gracefully (synthetic asset returned)', async () => {
    ...
    const processor = new KnowledgeProcessor(failChain as unknown as import('@supabase/supabase-js').SupabaseClient, bus);
    const result = await processor.process(baseInput(), RICH_CONTENT, 'asset-001');
