...
import {
  ...
  sitIsolationAllowed,
  checkNoDirectDb,
  DOMAIN_OWNERSHIP_RESTRICTED_DIRS,
...
// ── RULE-PIPELINE-NO-DIRECT-DB (Completion Mission — Gap Analysis G-2) ────────
//
// Mechanically enforces the fix for G-2: pipeline/, knowledge/, blueprint/,
// and context/ must never import '@supabase/supabase-js' directly — every
// write/read must route through the domain that owns the table. See
// checkNoDirectDb()'s own docblock in check-boundaries.mjs for why
// RULE-IOS-ISOLATION alone couldn't catch this.

describe('checkNoDirectDb() — RULE-PIPELINE-NO-DIRECT-DB against fixture files', () => {
  it('covers exactly the four restricted directories', () => {
    expect(DOMAIN_OWNERSHIP_RESTRICTED_DIRS.sort()).toEqual(
      ['blueprint', 'context', 'knowledge', 'pipeline'].sort(),
    );
  });

  it('reports zero violations when no restricted directory exists', () => {
    mkdirSync(join(dir, 'domains'));
    write('domains/UserIntelligenceDomain.ts', `import type { SupabaseClient } from '@supabase/supabase-js';\n`);
    const violations = checkNoDirectDb(dir, dir);
    expect(violations).toEqual([]);
  });

  it('reports zero violations for a clean pipeline/ fixture (domain-routed)', () => {
    mkdirSync(join(dir, 'pipeline'));
    writeFileSync(
      join(dir, 'pipeline', 'HypothesisEngine.ts'),
  ...
  it('flags a direct @supabase/supabase-js import inside pipeline/, with correct file/line', () => {
    mkdirSync(join(dir, 'pipeline'));
    writeFileSync(
      join(dir, 'pipeline', 'Regression.ts'),
      `// header\nimport type { SupabaseClient } from '@supabase/supabase-js';\n`,
    );
    const violations = checkNoDirectDb(dir, dir);
    expect(violations).toHaveLength(1);
    expect(violations[0]!.file).toBe(join('pipeline', 'Regression.ts'));
    expect(violations[0]!.line).toBe(2);
    expect(violations[0]!.specifier).toBe('@supabase/supabase-js');
  });

  it('flags violations across all four restricted directories independently', () => {
    for (const d of ['pipeline', 'knowledge', 'blueprint', 'context']) {
  ...
  it('does not flag @supabase/supabase-js imports outside the restricted directories', () => {
    mkdirSync(join(dir, 'domains'));
    writeFileSync(
      join(dir, 'domains', 'UserIntelligenceDomain.ts'),
      `import type { SupabaseClient } from '@supabase/supabase-js';\n`,
    );
    const violations = checkNoDirectDb(dir, dir);
    expect(violations).toEqual([]);
