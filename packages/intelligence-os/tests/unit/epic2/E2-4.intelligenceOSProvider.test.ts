...
describe('IntelligenceOSProvider — delegation', () => {
  ...
  it('ingestWorkspaceConfiguration() delegates with the same input and returns the same id (closes audit finding D-4)', async () => {
    const { provider, intelligenceOS } = makeProvider();
    const spy = vi.spyOn(intelligenceOS, 'ingestWorkspaceConfiguration').mockResolvedValue('asset-config-1');
    const input = { workspaceId: 'ws-1', identityConfiguration: { brandName: 'Acme' } };
    const id = await provider.ingestWorkspaceConfiguration(input);
