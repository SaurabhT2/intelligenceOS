...
CREATE TABLE intelligence.artifact_blueprints (
  ...
  -- Completion Mission (post-Epic-2 session): the two Epic 2 / E2-1-T1
  -- fields that are genuine row-level blueprint state get columns here.
  -- `build_duration_ms` deliberately does NOT get one — per the team's own
  -- IMPLEMENTATION_STATUS.md reasoning, it's a performance metric better
  -- suited to an observability pipeline than a row-level audit column, and
  -- is still returned to the caller only.
  degraded                     BOOLEAN,
...
CREATE INDEX intelligence_artifact_blueprints_expiry
  ...
  WHERE expires_at < now() + INTERVAL '7 days';  -- for TTL cleanup job
