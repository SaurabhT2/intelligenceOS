...
export interface IIntelligenceProvider {
  ...
  /**
   * Ingests explicit, admin-declared workspace configuration (voice
   * overrides, identity declarations, compliance requirements) as
   * Knowledge — ADR-003 §2.4. Persisted as a `KnowledgeAsset`, read back
   * by `context/ContextBuilder.ts` ahead of Learning-derived voice/identity.
   *
   * Promoted to this interface during the Completion Mission session
   * (previously a concrete-class-only method on `IntelligenceOS`, per
   * Architectural Rule 7's "public-contract additions require a separate,
   * considered decision"). That review is this addition: the method's
   * existing implementation and contract shape were already stable and
   * exercised by tests; the only open question — whether it belonged on
   * the versioned public surface — is resolved here, closing the
   * Completion Mission audit's finding D-4 (a real, correct method with
   * no reachable producer). See `docs/IMPLEMENTATION_STATUS.md`'s
   * "Versioning policy" and `CHANGELOG.md` for the record of this change.
   *
