...
`docs/adr/ADR-002-apps-runtime-layer.md` for why the platform is split into
...
|---|---|---|
| `packages/intelligence-os` | Domains, pipelines, blueprint generation, knowledge extraction, database access, the HTTP route/auth logic itself (`createCognitionHttpServer`) | Environment variables, process lifecycle, deployment platform config, `.listen()` calls |
...
4. **`vercel.json`'s rewrite** sends `/v1/cognition/:path*` to the single
   ...
   `@intelligence-os/core`'s `createCognitionHttpServer` for every request —
   the same handler-construction function the traditional server
   ...
   `createCognitionHttpServer` returns a Node `http.Server`; the Vercel
   function never calls `.listen()` on it (there's no persistent process to
   bind a port on) — instead it emits a `'request'` event on that server
   per invocation, which dispatches into the exact same routing/auth logic
   `.listen()` would have used. See `api/cognition.ts`'s own docblock for
   why this works without reimplementing or duplicating any routes.
...
`createCognitionHttpServer` supports a sixth, optional route
(`POST /v1/knowledge/ingest`) when passed a `KnowledgeIngestPort`. As of
the Completion Mission session, both `apps/api/src/server.ts` and
`apps/api/api/cognition.ts` pass `intelligenceOS` for this argument (it
already structurally satisfies the port), so this route is live on both
the local and hosted deployments this document describes. Before that
session, neither entrypoint passed one and the route returned `501` in
every real deployment — see `IMPLEMENTATION_STATUS.md` for the fix.
