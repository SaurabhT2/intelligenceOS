...
| `RelationshipIntelligenceDomain` | `relationships`, named rows in `audience_profiles` | — | Every method throws `DomainNotActivatedError`. Activates when a user has ≥3 external artifacts with named recipients, or via an explicit onboarding signal — see §6 Taxonomy note. Nothing else in the codebase calls into this domain yet. |
...
**One placement gap exists today:** `packages/intelligence-os`'s own package-root `AGENT_CONTEXT.md` currently lives at the repository root instead of inside the package it describes. Tracked in `IMPLEMENTATION_STATUS.md`'s Known Issues rather than silently corrected here.
